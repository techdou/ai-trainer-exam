...
- **用户管理**：建号、重置密码、停用/启用、改角色；学校管理员只能管理本校用户、只能分配学员/教师/监考/题库类角色，无法操作超级管理员账号。
- **学员名册导入**：上传 Excel 名册批量建号，初始密码为"身份证后六位 + 机构级随机后缀"（后缀存于系统设置，避免名册即密码本）；带 ZIP/OLE 文件头校验与 5MB 上限。
- **职责分离**：题库编辑不能审核自己提交的题目。
- **题库管理**：练习题库与考试题库独立管理，组卷时从练习库冻结快照到试卷；支持 DOCX 批量导入。
