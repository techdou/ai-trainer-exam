/**
 * 提示词描述题种子：prompt_description 题型。
 * 学员根据图片素材，用自然语言撰写提示词描述图片内容。
 * 评分按关键词命中率（支持 | 分隔同义词）。
 * 运行：pnpm tsx scripts/db/seed-prompt-description.mts
...
const questions: PromptDescription[] = [
  {
...
try {
  ...
  for (const q of questions) {
    ...
    await client.query(
      ...
      [organizationId, q.stem, options, JSON.stringify(answerKey), q.kp, q.difficulty],
    );
    practiceInserted++;
