...
export async function GET(request:NextRequest){
 try{
  ...
  const rows=await dbQuery<TeacherExamRow>(`SELECT s.id,s.title,s.cohort_id,s.exam_start_at,s.exam_end_at,s.late_entry_minutes,s.status,s.paper_id,s.created_at,p.title paper_title,p.duration_minutes
