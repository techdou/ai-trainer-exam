...
export async function GET(request:NextRequest){
 try{const user=await requireRole(request,['super_admin','school_admin']);const scopedOrg=organizationScope(user);const rows=await dbQuery<{id:string;name:string;code:string;contact:string|null;status:string;created_at:string;cohort_count:string;student_count:string}>(
 ...
 FROM organizations o WHERE o.deleted_at IS NULL AND ($1::text IS NULL OR o.id=$1) ORDER BY o.created_at DESC`,scopedOrg);
