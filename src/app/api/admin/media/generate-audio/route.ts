...
export async function POST(request:Request){
 try{
  ...
  const job=await dbOne<{id:string}>(`INSERT INTO media_generation_jobs(organization_id,media_kind,provider,status,prompt,params,created_by,created_at,updated_at)
   VALUES($1,'audio','pending','processing',$2,$3,$4,NOW(),NOW()) RETURNING id`,user.organizationId,body.text,{speaker:body.speaker,category:body.category,targetBank:body.targetBank},user.id);
  try{
   const generated=await generateAudio(body.text,body.speaker,request.headers);const key=mediaObjectKey(user.organizationId,'audio',generated.extension);await uploadObject(key,generated.buffer,generated.contentType);const checksum=sha256(generated.buffer);
   const asset=await dbOne<{id:string}>(`INSERT INTO asset_manifests(organization_id,media_kind,object_key,checksum,version,status,category,transcript,meta,job_id,created_at,updated_at)
    VALUES($1,'audio',$2,$3,1,'draft',$4,$5,$6,$7,NOW(),NOW()) RETURNING id`,user.organizationId,key,checksum,body.category,body.text,{targetBank:body.targetBank,contentType:generated.contentType,provider:generated.provider,...generated.metadata},job!.id);
