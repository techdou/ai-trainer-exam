...
import { graderIdForTaskType, gradeByType } from '@/server/grading';
...
async function loadSourceItem(item: z.infer<typeof itemSchema>, organizationId: string): Promise<SourceItem | null> {
  if (item.itemType === 'question') {
    ...
    if (!row) return null;
    const graderId = row.question_type === 'true_false' ? 'true_false' : 'single_choice';
    ...
    const probe = gradeByType(graderId, row.question_type === 'true_false' ? { answer: false } : { selectedOption: 'A' }, answerKey);
