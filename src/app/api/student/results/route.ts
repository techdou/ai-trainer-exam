...
export async function GET(req:NextRequest){
 try{
  ...
  // 学员端只见结论(合格/不合格), 不下发具体分数: 发证与成绩复核由机构线下流程管理。
  const rows=await dbQuery<{id:string;schedule_id:string;schedule_title:string;passed:boolean;status:string;adjusted:boolean;created_at:string}>(
   `SELECT sc.id,sc.schedule_id,s.title schedule_title,sc.passed,sc.status,
  ...
  return ok(rows.map(r=>({id:r.id,scheduleId:r.schedule_id,scheduleTitle:r.schedule_title,passed:r.passed,status:r.status,adjusted:r.adjusted,createdAt:r.created_at})));
